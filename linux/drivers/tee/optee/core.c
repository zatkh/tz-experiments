...
/*** call smc test ***/
static bool optee_invoke_test_smc(optee_invoke_fn *invoke_fn)
{
	struct arm_smccc_res res;


	invoke_fn(OPTEE_SMC_CALL_TEST, 0, 0, 0, 0, 0, 0, 0, &res);
...
static int optee_probe(struct platform_device *pdev)
{
	...
	if(optee_invoke_test_smc(invoke_fn))
		dev_info(&pdev->dev, "optee_invoke_test_smc successful\n");
