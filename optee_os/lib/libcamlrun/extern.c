...
static inline void _write(int c)
{
...
static void extern_rec(value v)
{
  ...
  if (Is_in_value_area(v) || caml_extern_allow_out_of_heap) {
    ...
    case String_tag: {
      ...
      if (len < 0x20) {
        _write(PREFIX_SMALL_STRING + len);
      } else if (len < 0x100) {
    ...
    default: {
      value field0;
      if (tag < 16 && sz < 8) {
        _write(PREFIX_SMALL_BLOCK + tag + (sz << 4));
      } else {
#ifdef ARCH_SIXTYFOUR
#ifdef WITH_PROFINFO
