...
{
   IMSG("[test_console]hey this is a new test enclave \n" );
   char c;
 
 	 IMSG("[test_console] Enter character: ");
   c = console_getc();
 
   IMSG("[test_console] Character entered: ");
   console_putc(c);
...
// ocall access to open syscall in nw
static TEE_Result ocall_open(uint32_t param_types __unused,TEE_Param params[TEE_NUM_PARAMS])
{
	...
	IMSG("[ocall_open], name is %s,len is %d",( char*) params[0].memref.buffer, len);
...
//	IMSG("[ocall_write], buf : %s,len is %d, fd : %d",( char*) params[1].memref.buffer, len,fd);
...
static TEE_Result ocall_read(uint32_t param_types __unused,TEE_Param params[TEE_NUM_PARAMS])
{
	...
	//IMSG("[ocall_read], buf : %s,len is %d, fd : %d",( char*)params[1].memref.buffer, params[1].memref.size,fd);
